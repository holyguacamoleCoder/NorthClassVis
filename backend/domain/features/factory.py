...
from domain.reduction import DimReduction
...
class FeatureFactory:
    ...
    def _initialize_features(self):
        self.preliminary_feature_calculator = PreliminaryFeatureCalculator(
            self.config.get_data_with_title_knowledge()
        )
        ...
        self.feature_knowledge = FinalFeatureCalculator(
            features, ["student_ID", "knowledge"]
        ).get_result()
        self.dim_reduction = DimReduction(self.feature_bonus)
        self.cluster_analysis = ClusterAnalysis(
            students_data=self.dim_reduction.get_transformed_data().to_dict(orient="index")
        )
