...
class QuestionRoutes:
    ...
    def get_timeline_data(self, title_id):
        timeline_data = qv.process_timeline_data(self.data_with_title_knowledge, title_id)
        return jsonify(timeline_data)

    def get_distribution_data(self, title_id):
        distribution_data = qv.process_distribution_data(self.data_with_title_knowledge, title_id)
    ...
    def get_question(self):
        ...
        if title_id is not None:
            ...
            title_data = {
                'title_id': title_id,
                'knowledge': self.data_with_title_knowledge.loc[self.data_with_title_knowledge['title_ID'] == title_id, 'knowledge'].iloc[0],
                'timeline': qv.process_timeline_data(self.data_with_title_knowledge, title_id),
                'distribution': qv.process_distribution_data(self.data_with_title_knowledge, title_id)
            }
            return jsonify([title_data])
        elif knowledge is not None:
            # 如果指定了知识点，则返回该知识点下所有题目的数据
            titles_data = qv.get_titles_data_by_knowledge(self.data_with_title_knowledge, knowledge, limit)
            return jsonify(titles_data)
        else:
            # 如果没有指定知识点或题目ID，则返回所有题目的数据
            all_titles_data = qv.get_all_titles_data(self.data_with_title_knowledge, limit)
