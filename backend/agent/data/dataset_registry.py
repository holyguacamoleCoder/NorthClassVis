...
def format_catalog_hint(session_id: str | None, *, tail: int = 5) -> str:
    ...
    if not rows:
        return ""
    lines = ["最近数据集（dataset_id → 行数/limit/轮次）:"]
    for item in rows:
        lim = (
            f"limit={item['query_limit']}"
            if item.get("query_limit") is not None
            else "全量"
        )
        lines.append(
            f"  {item['dataset_id']}: {item['result_rows']} 行, {lim}, "
            f"user_turn={item['user_turn']}"
        )
    return "\n".join(lines)
    ...
    *,
    ...
    if items:
        ...
        meta["next_step"] = {
            ...
            "examples": [
                {
                    "input": {"dataset_id": newest["dataset_id"]},
                    "bind": "chain",
                    "note": "续算最近数据集（切片/本回合）",
                },
                {
