...
# 测试 load_submissions_by_classes 函数
# 创建一个临时的CSV文件用于测试
@pytest.fixture
...
# 测试 load_submissions_by_classes 函数
def test_load_submissions_by_classes(test_class_list):
    class_list, tmp_path = test_class_list
    df = load_submissions_by_classes(tmp_path, class_list)
    expected_df = pd.DataFrame({"id": [1, 2], "title_ID": [101, 102]})
    assert df.equals(expected_df)

# 测试 merge_dataframes_or_files 函数
@pytest.fixture
...
def test_csv2(tmp_path):
    ...
    return str(file_path)
def test_merge_dataframes_or_files(test_csv1, test_csv2):
    merged_df = merge_dataframes_or_files(
        left_path=test_csv1,
        right_path=test_csv2,
        on="title_ID",
    )
